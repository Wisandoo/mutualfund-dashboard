...
export default function Home() {
  ...
  const [expandedHoldingIndex, setExpandedHoldingIndex] = useState<number | null>(null);
      ...
      <main className="min-h-screen bg-gray-50 p-8 text-gray-800">
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            {/* 1. Grafik Lingkaran: Alokasi Aset (Radial Bar Chart) */}
          <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200 flex flex-col">
            ...
            <div className="relative h-60 w-full flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <RadialBarChart 
                  cx="50%" cy="50%" innerRadius="45%" outerRadius="100%" barSize={12} 
                  startAngle={90} endAngle={-270}
                  data={(fundDetails.portfolioAllocations?.length ? [...fundDetails.portfolioAllocations] : [{ name: "Belum ada data", percentage: 100 }])
                    .map((item, index) => ({
                      ...item,
                      // Nilai absolut agar grafik tetap menggambar baris walau nilainya negatif
                      // Math.min memastikan bar maksimal melingkar 100% penuh
                      displayValue: Math.min(Math.abs(item.percentage), 100),
                      // Palette warna statis: Hijau, Biru, Kuning, Ungu, Merah
                      fill: ['#22c55e', '#3b82f6', '#f59e0b', '#a855f7', '#ef4444'][index % 5]
                    }))} 
                >
                  <PolarAngleAxis type="number" domain={[0, 100]} angleAxisId={0} tick={false} />
                  <RadialBar background={{ fill: '#f3f4f6' }} dataKey="displayValue" cornerRadius={10} />
                  
                  {/* Tooltip dimodifikasi agar menampilkan nilai asli (termasuk negatifnya) */}
                  <Tooltip 
              ...
              {/* Tanda 0%, 50%, 100% */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 text-xs text-gray-400 font-medium">0%</div>
              <div className="absolute bottom-6 right-1/4 translate-x-5 text-xs text-gray-400 font-medium">50%</div>
              <div className="absolute top-1/2 left-1/4 -translate-x-5 -translate-y-1/2 text-xs text-gray-400 font-medium">100%</div>
            </div>

            {/* Custom Legend bergaris mirip desain */}
            <div className="mt-4 border-t border-gray-100 flex-1">
              {(fundDetails.portfolioAllocations?.length ? [...fundDetails.portfolioAllocations] : [{ name: "Data Tidak Tersedia", percentage: 0 }])
                .map((item, index) => {
                  ...
                  return (
                  <div key={index} className="flex items-center justify-between py-3.5 border-b border-gray-100 last:border-0">
                    <div className="flex items-center gap-3">
                      <span className="h-4 w-4 rounded-full" style={{ backgroundColor: colors[index % colors.length] }}></span>
            ...
            <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
              ...
              <div className="mt-2 divide-y divide-gray-100 h-80 overflow-y-auto pr-2 custom-scrollbar">
                  {fundDetails.topHoldings?.map((holding: any, index: number) => {
                    const isExpanded = expandedHoldingIndex === index;

                    const isValid = (val: any) => {
                      if (val === null || val === undefined) return false;
                      const strVal = String(val).trim().toLowerCase();
                      return strVal !== '' && strVal !== '-' && strVal !== 'nan';
                    };

                    // Pengecekan per field
                    const hasCode = isValid(holding.code);
                    const hasMaturity = isValid(holding.maturityDate);
                    const hasType = isValid(holding.type);
                    const hasSector = isValid(holding.sector);
                    const hasCoupon = isValid(holding.couponFrequency);
                    const hasInterest = isValid(holding.interest);
                    const hasRating = isValid(holding.rating);
                    const hasIssuer = isValid(holding.issuer);

                    const hasAnyData = hasCode || hasMaturity || hasType || hasSector || hasCoupon || hasInterest || hasRating || hasIssuer;
                    const showIdxLink = isValid(holding.url) && holding.type?.toUpperCase() === 'EQUITY';
                    
                    return (
                      <div key={index} className="flex flex-col py-3 group border-b border-gray-50 last:border-0">
                        <div 
                          className="flex items-center justify-between cursor-pointer"
                          onClick={() => setExpandedHoldingIndex(isExpanded ? null : index)}
                        >
                          ...
                          <div className="flex items-center gap-3 text-sm font-bold text-gray-900 justify-end w-1/4">
                            {holding.percentage}%

                            <svg 
                              className={`w-4 h-4 text-gray-400 transition-transform duration-200 ${isExpanded ? 'rotate-180 text-blue-600' : ''}`} 
                        ...
                        {isExpanded && (
                          <div className="mt-3 p-4 bg-gray-50/80 rounded-lg border border-gray-100 text-sm animate-in fade-in slide-in-from-top-2 duration-200">
                            {!hasAnyData ? (
                              <div className="flex items-center justify-center py-4">
                                <p className="text-gray-500 font-medium italic">Data Tidak Tersedia</p>
                              </div>
                            ) : (
                              // Tampilan dinamis hanya untuk field yang memiliki data
                              <div className="grid grid-cols-2 gap-y-4 gap-x-4">
                                {hasCode && (
                                  <div>
                                    <p className="text-xs text-gray-500 mb-1">Kode</p>
                                    <p className="font-semibold text-gray-800">{holding.code}</p>
                                  </div>
                                )}
                                
                                {hasMaturity && (
                                  <div>
                                    <p className="text-[13px] text-gray-500 mb-1">Tanggal Jatuh Tempo</p>
                                    <p className="text-sm font-semibold text-gray-800">{holding.maturityDate}</p>
                                  </div>
                                )}
                                
                                {hasType && (
                                  <div>
                                    <p className="text-[13px] text-gray-500 mb-1">Tipe</p>
                                    <p className="text-sm font-semibold text-gray-800">{holding.type}</p>
                                  </div>
                                )}
                                
                                {hasSector && (
                                  <div>
                                    <p className="text-[13px] text-gray-500 mb-1">Sektor</p>
                                    <p className="text-sm font-semibold text-gray-800">{holding.sector}</p>
                                  </div>
                                )}
                                
                                {hasCoupon && (
                                  <div>
                                    <p className="text-[13px] text-gray-500 mb-1">Kupon</p>
                                    <p className="text-sm font-semibold text-gray-800">
                                      {holding.couponFrequency === 'Q' ? '3 MONTHS' : 
                                      holding.couponFrequency === 'SA' ? '6 MONTHS' : 
                                      holding.couponFrequency === 'A' ? '12 MONTHS' : 
                                ...
                                {hasInterest && (
                                  <div className="flex flex-col">
                                    <span className="text-[13px] text-gray-500 mb-1">Interest</span>
                                    <span className="text-sm font-semibold text-gray-800">
                                      {Number(holding.interest).toFixed(2)}%
                                    </span>
                                  </div>
                                )}
                                ...
                                {hasRating && (
                                  <div>
                                    <p className="text-[13px] text-gray-500 mb-1">Peringkat Perusahaan</p>
                                    <p className="text-sm font-semibold text-gray-800">{holding.rating}</p>
                                  </div>
                                )}
                                
                                {hasIssuer && (
                                  <div>
                                    <p className="text-[13px] text-gray-500 mb-1">Penerbit</p>
                                    <p className="text-sm font-semibold text-gray-800">{holding.issuer}</p>
                                  </div>
                                )}
                              </div>
                            )}

                            {/* Link tambahan yang didapat dari KSEI Service */} 
                            {hasAnyData && showIdxLink && (
                              <div className="mt-4 pt-3 border-t border-gray-200 flex gap-4">
                                {holding.url && (
                                  <a href={holding.url} target="_blank" rel="noopener noreferrer" className="text-xs font-bold text-blue-600 hover:text-blue-800 flex items-center gap-1">
                                    Profil IDX ↗
                                  </a>
                                )}
                              </div> 
